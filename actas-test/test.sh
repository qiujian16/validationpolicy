...
curl -H"Authorization:Bearer $TOKEN" -H"ActAs-User:bob" -k https://localhost:6443/api/v1/nodes

echo "sa actas user to get service will fail since the user does not have the permission"

curl -H"Authorization:Bearer $TOKEN" -H"ActAs-User:bob" -k https://localhost:6443/api/v1/services

echo "sa actas user to get pods succeeds since both have the permission"

curl -H"Authorization:Bearer $TOKEN" -H"ActAs-User:bob" -k https://localhost:6443/api/v1/pods
